...
@interface AnswerDetailViewController ()<UITableViewDelegate,UITableViewDataSource,XHInputViewDelagete,TYAttributedLabelDelegate>

@property (nonatomic, weak) UITableView *tableView;
//底部输入框
@property (nonatomic, weak) UIView *bottomBgView;

@property (nonatomic, strong) DetailAnswerTopView *topView;
...
- (void)viewDidLoad {
    ...
    if(self.answerDetailModel != nil){
        self.answerId = [NSString stringWithFormat:@"%@",self.answerDetailModel.answerId];
    }
...
-(void)createNoDataTipView{
    ...
    noDetailTipView.tapClick = ^(NoDataTipType tipType) {
        if (tipType == NoDataTipTypeNoData) {
            [[NSNotificationCenter defaultCenter] postNotificationName:KNotificationDataRemoved object:nil userInfo:nil];
...
-(void)ShareQuestionItem{
    if (self.detailTopModel == nil) {
        return;
    }
    NSString *shareUrl = [NSString stringWithFormat:@"http://%@/appAnswer/share/%@/%@",RequestHeader,self.questionId,self.answerId];
    NSString *shareTitle = [NSString stringWithFormat:@"%@",self.detailTopModel.title];
    NSString *shareDes = [NSString stringWithFormat:@"%@",self.detailTopModel.answerContent];
    [CustomShareUI shareWithUrl:shareUrl Title:shareTitle DesStr:shareDes];
}
#pragma mark - 请求数据
-(void)requestGroupDetailTopicDetailData{
    WeakSelf
    [[RequestManager sharedInstance]postQAAnswerDetailWithUserID:[StoreTool getUserID] QuestionId:self.questionId AnswerId:self.answerId Success:^(id responseData) {
        ...
        if ([dict[@"code"] isEqualToString:@"0000"]) {
            ...
            if ([TipDic[@"flag"] isEqualToString:@"true"]) {
                ...
                DetailQuestionModel *detailAllModel = [DetailQuestionModel yy_modelWithDictionary:TipDic[@"appAnswer"]];
                strongSelf.detailTopModel = detailAllModel;
                
                [self reloadTableHeaderViewData:detailAllModel];
            } else {
                if ([TipDic[@"code"] isEqualToString:@"6011"] || [TipDic[@"code"] isEqualToString:@"6012"]) {
...
-(void)createAlertVCWithNoData{
    ...
    [self.noDetailTipView changeTipLabel:KAnswerDetailDataRemoved];
}

#pragma mark - 请求评论列表
-(void)requestGroupDetailCommentListDataWith:(NSInteger)page{
    WeakSelf
    [[RequestManager sharedInstance]postQAAnswerDetailWithUserID:[StoreTool getUserID] AnswerId:self.answerId ListPage:page Success:^(id responseData) {
        NSDictionary *dict = responseData;
...
-(void)postAddComment:(NSString *)commentStr commentId:(NSString *)commentId linkId:(NSString *)linkId toUserId:(NSString *)toUserId commentType:(CommentType)commentType{
    WeakSelf
    [[RequestManager sharedInstance]postQAAnswerDetailCommentAddWithUserId:[StoreTool getUserID] ToUserId:toUserId AnswerId:self.answerId CommentId:commentId LinkId:linkId CommentContent:commentStr Success:^(id responseData) {
        NSDictionary *dict = responseData;
        if ([dict[@"code"] isEqualToString:@"0000"]) {
            StrongSelf
            NSDictionary *TipDic = dict[@"data"];
...
-(void)requestGroupDetailLikeData{
    ...
    WeakSelf
    [[RequestManager sharedInstance]postQAQuestionDetailLikeWithUserID:[StoreTool getUserID] AnswerId:self.answerId Success:^(id responseData) {
        NSDictionary *dict = responseData;
        if ([dict[@"code"] isEqualToString:@"0000"]) {
            StrongSelf
            NSDictionary *TipDic = dict[@"data"];
...
#pragma mark - UI
-(void)createUI{
    UITableView *tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, Width_Window, Height_View_SafeArea-44-50) style:UITableViewStyleGrouped];
    ...
    DetailAnswerTopView *topView = [[DetailAnswerTopView alloc]initWithFrame:CGRectMake(0, 0, Width_Window, 260)];
    topView.likeBtnClickBlock = ^{
      //点赞回调
    ...
    //赋值
//    topView.titleLabel.text = [NSString stringWithFormat:@"%@",self.questionDetailModel.title];
    [self reloadTableHeaderViewData:self.answerDetailModel];
}
-(void)reloadTableHeaderViewData:(DetailQuestionModel *)model{
...
-(void)addInputControl{
    UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(0, Height_Window-50-Height_View_HomeBar-Height_Statusbar-44, Width_Window, 50+Height_View_HomeBar)];
...
-(void)showTextField:(UIControl *)sender{
    ...
    [XHInputView showWithStyle:InputViewStyleDefault configurationBlock:^(XHInputView *inputView) {
        ...
        /** 带进去的内容 */
        inputView.defaultText = self.currentCommentStr;
        /** 占位符文字 */
        ...
        if(text.length){
            NSLog(@"输入的信息为:%@",text);
            [self postAddComment:text commentId:self.commentId linkId:self.linkId toUserId:self.toUserId commentType:self.commentType];
...
-(void)xhInputViewKeyboardWillShow:(CGFloat)inputOriginY{
    if (self.commentType == CommentTypeReply) {
        [self scrollViewDropDown:inputOriginY];
    }
}

-(void)xhInputViewKeyboardWillHide:(CGFloat)inputOriginY{
    if (self.commentType == CommentTypeReply) {
...
-(void)scrollViewDropDown:(CGFloat)originY{
    ...
    CGFloat heightToBottom = Height_Window - Height_View_HomeBar - rect.origin.y - rectInTableView.size.height;
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    DetailAnswerHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:@"detailAnswerHeaderView"];
