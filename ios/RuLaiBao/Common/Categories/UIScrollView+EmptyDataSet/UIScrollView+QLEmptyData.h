...
typedef NS_ENUM(NSInteger, EmptyDataSetType) {
    EmptyDataSetTypeNetwork  = 100010,    // 无网络
    EmptyDataSetTypeOrder    ,            // 无订单
    EmptyDataSetTypeSearchList,           //无搜索数据
    
};

typedef void (^loadingBlock)(void);

@interface UIScrollView (QLEmptyData)<DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
/*!
 展示的展位图的类型以及调用的方法
 */
@property (nonatomic, assign) EmptyDataSetType emptyDataSetType;
