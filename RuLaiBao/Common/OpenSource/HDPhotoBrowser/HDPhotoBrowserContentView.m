...
@interface HDPhotoBrowserContentView ()<UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
/** 加载进度视图 */
@property (nonatomic, strong) HDPhotoBrowserLoadingView *loadingView;


@end

@implementation HDPhotoBrowserContentView
    ...
    [_imageView sd_setImageWithURL:url
                  placeholderImage:placeholder
                         ...
                         completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                             ...
                             // 优化超宽图片的显示
                             if (aspectRatio > 1.5) {
                                 strongSelf.scrollView.maximumZoomScale *= aspectRatio / 1.5;
                             }
...
#pragma mark - 懒加载
/** 加载进度视图 */
- (HDPhotoBrowserLoadingView *)loadingView {
    if (_loadingView == nil) {
        HDPhotoBrowserLoadingView *loadingView = [[HDPhotoBrowserLoadingView alloc] init];
...
- (UIButton *)reloadButton {
    if (_reloadButton == nil) {
        ...
        [button setTitle:@"原图加载失败,请点击重新加载" forState:UIControlStateNormal];
