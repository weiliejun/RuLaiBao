...
- (void)creataCell{
    ...
    [popularFeeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.contentView.mas_right).offset(-15);
        make.top.equalTo(nameLabel.mas_bottom).offset(25);
        make.width.mas_equalTo(@(Width_Window/3-20));
        make.height.mas_equalTo(@20);
    }];
...
- (void)setValueforCell:(HomeViewModel *)homeModel{
    self.nameLabel.text = [NSString stringWithFormat:@"%@",homeModel.name];
    self.companyLabel.text = [NSString stringWithFormat:@"%@",homeModel.companyName];
    self.insuranceFeeLabel.text = [NSString stringWithFormat:@"%@元起",homeModel.minimumPremium];
    
    //判断登录和认证状态
    //计算text的layout
    if ([StoreTool getLoginStates]) {//登录
        if ([homeModel.checkStatus isEqualToString:@"success"]) {//success认证成功
            self.leftImage.hidden = YES;
            //推广费
            self.popularFeeLabel.text = [NSString stringWithFormat:@"%@%%",homeModel.promotionMoney];
        ...
        if ([homeModel.checkStatus isEqualToString:@"success"]) {
            //success认证成功
    ...
    //判断有无标语
    if (![homeModel.recommendations isEqualToString:@""]) {
        //有标语
        self.sloganLabel.hidden = NO;
        self.sloganLabel.text = [NSString stringWithFormat:@"%@",homeModel.recommendations];
